...
public class Account {
    ...
    public void withdraw(double amount, String ssn) throws NegativeAmountException, InsufficientBalanceException, SsnInvalidException {
        try {
            ...
            if (!isSsnValid(ssn)) throw new SsnInvalidException("The SSN= " + ssn + " does not match");

            balance -= amount;
            //audit trail: who, when, what, initial balance, final balance
        } catch (NegativeAmountException | InsufficientBalanceException | SsnInvalidException e) {
